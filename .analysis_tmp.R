...
#| fig-width: 3
#| fig-height: 5
#| fig-cap: "Kōura presence, CPUE, and BPUE across lakes. The upper panel shows the proportion of sampled sites with kōura present in each lake, with error bars indicating 95% binomial confidence intervals. The middle and lower panels show the distribution of kōura CPUE and BPUE, respectively, across lakes using boxplots (median, interquartile range, and range)."
...
KCPUE_plot <- plot_koura_stats(M_C_data, "Weighted_CPUE_Kōura", "Kōura CPUE\n(log scale)", type = "continuous", show_x_title = FALSE)
KBPUE_plot <- plot_koura_stats(M_C_data, "Weighted_BPUE_Kōura", "Kōura BPUE\n(log scale)", type = "continuous", show_x_title = TRUE)
...
# Tag first smooth panel to label the whole top group as "a)"
plots_4[[1]] <- plots_4[[1]] + labs(tag = "a)")
...
p_pres_roc <- ggplot(roc_df, aes(fpr, tpr)) +
  ...
  labs(x = "False positive rate", y = "True positive rate", tag = "b)") +
  base_theme_bw
...
Calibration_plot_occ <- ggplot(calib_data_occ, aes(mean_pred, obs_rate)) +
  ...
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", colour = "black", linewidth = 0.4) +
  labs(x = "Mean predicted probability", y = "Observed proportion", tag = "c)") +
  base_theme_bw
...
cpue_smooth_plots[[1]] <- cpue_smooth_plots[[1]] + labs(tag = "a)")
...
bpue_smooth_plots[[1]] <- bpue_smooth_plots[[1]] + labs(tag = "a)")
