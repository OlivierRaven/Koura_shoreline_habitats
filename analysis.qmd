...
biotic_vars <- data.frame(
  `Variable`= c("Lake identity (LID)", "Substrate index", "Slope", "Riparian vegetation", "Overhanging trees", "Wood cover", "Emergent and submerged macrophytes", "Temperature", "Dissolved oxygen", "Specific conductivity", "pH", "Fish presence"),
...
report_lrt <- function(lrt_obj, term = "Lake") {
  subscript_digits <- chartr("0123456789", "₀₁₂₃₄₅₆₇₈₉", as.character(lrt_obj[term, "Df"]))
  chisq <- formatC(lrt_obj[term, "LRT"], format = "f", digits = 2)
  p_val <- formatC(lrt_obj[term, "Pr(>Chi)"], format = "f", digits = 3)
  sprintf("χ²%s = %s, *p* = %s", subscript_digits, chisq, p_val)
}
