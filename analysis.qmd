...
pred_labels <- c(
  ...
  Overhanging_trees    = "Overhanging trees (%)",
  Wood_cover           = "Wood cover (%)",
  Emergent_Native      = "Emergent native vegetation (%)",
  Submerged_Non_Native = "Submerged non-native vegetation (%)",
  Submerged_Native     = "Submerged native vegetation (%)"
)
...
raw_long <- Monitoring_CPUE_data %>%
  ...
  dplyr::mutate(
    ...
    Response        = dplyr::recode(Response,
      "Presence_Kōura"        = "Presence (0/1)",
      "Weighted_CPUE_Kōura"   = "CPUE",
      "Weighted_BPUE_Kōura"   = "BPUE"
    ),
    Response = factor(Response, levels = c("Presence (0/1)", "CPUE", "BPUE")),
    is_retained = Predictor %in% retained_vars
  )

# Strip label face: bold for retained predictors
strip_faces <- ifelse(levels(raw_long$Predictor_label) %in% pred_labels[retained_vars],
                      "bold", "plain")

fig_raw_all <- ggplot(raw_long, aes(x = Predictor_value, y = Response_value)) +
  geom_jitter(
    data   = ~ dplyr::filter(.x, Response == "Presence (0/1)"),
  ...
  facet_grid(Response ~ Predictor_label, scales = "free", switch = "y") +
  labs(x = NULL, y = NULL) +
  base_theme_bw +
  theme(
    strip.text.x      = element_text(size = 6,
                                     face = strip_faces[match(
                                       levels(raw_long$Predictor_label),
                                       levels(raw_long$Predictor_label))]),
...
ggsave(file.path(out_dir, "fig-raw-data-all-predictors.png"),
       fig_raw_all, width = 8, height = 10, dpi = 300)
...
# kable_styling()/group_rows() emit HTML (bootstrap) markup, which trips
# Quarto's "HTML output found in document targeting docx/pdf output" check
# when this notebook is knitted from index.qmd's [start] chunk for non-HTML formats.
if (knitr::is_html_output()) {
  gam_models_table <- gam_models_table |>
    kable_styling(
