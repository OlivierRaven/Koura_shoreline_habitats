...
\makeatother
\hypersetup{
  pdftitle={Where do Kōura Live?},
  pdfauthor={Olivier V. Raven; Francis J. Burdon; Ian A. K. Kusabs; Robin Holmes; Deniz Özkundakci},
  pdfkeywords={\xmpquote{Rotorua Te Arawa Lakes}, \xmpquote{North
Island}, \xmpquote{Habitat structure}, \xmpquote{Thermal
stress}, \xmpquote{Generalised additive mixed
models}, \xmpquote{Paranephrops planifrons}},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\makeatother
\subtitle{Littoral habitat structure influences freshwater crayfish
...
Aotearoa's North Island, kōura populations have declined by 76\% and
91\%, respectively, over the past two decades
...
largely focussed on deeper offshore habitats
...
typically vary from 4°C to 24°C. The lakes span a wide range of
...
in Lakes Rotorua and Rotoiti at time of sampling
(\citeproc{ref-Kusabs2026}{Kusabs et al., 2026}).
...
(\emph{n} = 12) was surveyed in each lake, with sites allocated
proportionally among the dominant shoreline habitat types. All selected
sites were fully accessible for fieldwork, and therefore an oversample
was not required. Within each habitat type, site locations were
generated using the Generalized Random Tessellation Stratified (GRTS)
design (\citeproc{ref-Stevens2004}{Stevens \& Olsen, 2004}) executed
using the spsurvey package in R (\citeproc{ref-Dumelle2023}{Dumelle et
al., 2023}; \citeproc{ref-RCoreTeam2025}{R Core Team, 2025}), producing
...
oxygen (mg/L), and conductivity (μS/cm), measured with a YSI ProSolo
...
al., 2015b}). Missing kōura weights due to failure of scale (\emph{n} =
81) were estimated using a log₁₀--log₁₀ length--weight regression fitted
to individuals with both measurements (\emph{n} = 240), following
standard approaches for weight length relationships in aquatic organisms
({\textbf{?@fig-length-weight-ch3}}) (\citeproc{ref-Froese2006}{Froese,
...
dissolved oxygen expressed as percentage saturation (DO\%) exceeded this
threshold and was excluded from further analyses, while dissolved oxygen
concentration (DO mg L\(^{-1}\)) was retained.
...
dissolved oxygen (mg/L), pH, specific conductivity (μS/cm), and the
...
stepwise backward elimination: terms were sequentially removed and the
model refitted until all remaining terms met the retention threshold
...
characteristics ({\textbf{?@tbl-an-env-fish-summary}}). Physical habitat
...
\subsubsection{Kōura occupancy}\label{kux14dura-occupancy}
...
explained 57.9\% of the deviance (adj. R² = 0.592) and retained positive
influences of riparian vegetation (\emph{p} = 0.026), substrate index
(\emph{p} = 0.001), specific conductivity (\emph{p} = 0.002) and a
...
shown as partial effects with 95\% confidence intervals. See {Fig. S3.2}
for raw data underlying modelled relationships. (B) Receiver operating
...
\subsubsection{Kōura CPUE}\label{kux14dura-cpue}

The full Gamma GAMM for positive CPUE explained 48.4\% of the deviance
(AIC = 122; adj. R² = 0.19; \emph{n} = 33). Stepwise selection
...
confidence intervals. See {Fig. S3.2} for raw data underlying modelled
...
\subsubsection{Kōura BPUE}\label{kux14dura-bpue}

The full Gamma GAMM for positive BPUE explained 97.4\% of the deviance
(AIC = 266; adj. R² = 0.76; \emph{n} = 33). Stepwise ML selection
...
See {Fig. S3.2} for raw data underlying modelled relationships. (B)
Predicted versus observed BPUE from the positive (Gamma) component of
the hurdle model, with predictions excluding the random lake effect. The
dashed 1:1 line indicates perfect agreement; annotated values report R²
and RMSE to summarise model fit.}
...
\subsubsection{Habitat associations with
...
pore spaces between coarse particles may favour adult kōura, while
smaller interstitial spaces within mixed substrates may provide refugia
for juveniles, suggesting that heterogeneous substrates support kōura
across life stages. However, mixed substrates containing both coarse and
fine particles can reduce pore space availability as smaller particles
infill void spaces between larger ones
...
\subsubsection{Fish associations as indicator of habitat
...
\subsubsection{Associations with physico-chemical
...
increasing above approximately 10 °C; temperatures exceeding 21 °C are
considered thermally stressful and may induce aestivation
(\citeproc{ref-Devcich1979}{Devcich, 1979};
...
conservation (\citeproc{ref-Meerhoff2022a}{Meerhoff \&
González-Sagrario, 2022}). As shoreline modification, sedimentation, and
invasive macrophyte expansion continue to reduce variation and
...
for freshwater crayfish populations in these systems. Accordingly,
...
Kusabs, I. A. K., B. J. Hicks, J. M. Quinn, \& D. P. Hamilton, 2015a.
...
Kusabs, I. A. K., \& J. M. Quinn, 2009. Use of a traditional {Māori}
...
Kusabs, I. A. K., J. M. Quinn, \& D. P. Hamilton, 2015b. Effects of
...
New Zealand}. Marine and Freshwater Research 66: 631--643.
