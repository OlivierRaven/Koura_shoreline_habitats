...
if (file.exists(.i)) cat(sprintf('<div class="ch-hero-wrap"><img class="ch-hero-img" src="%s" alt=""><div class="ch-hero-fade"></div><div class="ch-hero-text"><span class="ch-eyebrow">Chapter 3</span><p class="ch-hero-title">Where do Kōura Live?</p></div></div>', knitr::image_uri(.i)))
```

# Chapter 3 Where do Kōura Live? {.unnumbered}
:::
...
if (!requireNamespace("sf", quietly = TRUE) || !requireNamespace("leaflet", quietly = TRUE)) {
  img <- if (file.exists("images/Natural_habitat_map.png"))
    "images/Natural_habitat_map.png"
  else
    "03_koura_shoreline_habitats/images/Natural_habitat_map.png"
  knitr::include_graphics(img)
} else {

ch3_dir      <- if (dir.exists("03_koura_shoreline_habitats")) "03_koura_shoreline_habitats" else "."
study_lakes  <- c("Rotorua", "Rotoiti", "Rotoehu", "Rotomā", "Ōkāreka")
catfish_lakes <- c("Rotorua", "Rotoiti")

all_lakes <- sf::st_read(
  file.path(ch3_dir, "data/raw/12_lakes.gpkg"), quiet = TRUE
) |>
  sf::st_make_valid() |>
...
all_lakes <- dplyr::mutate(all_lakes,
  ...
  dplyr::filter(lake_name %in% study_lakes) |>
  dplyr::mutate(catfish = lake_name %in% catfish_lakes)

site_info <- readr::read_csv(
  file.path(ch3_dir, "data/derived/Site_info.csv"), show_col_types = FALSE
) |>
  dplyr::distinct(site_id, lake, lat, lon, DHT)

dht_colors <- c(
  "Rocky"               = "#808080",
  "Sandy"               = "#d4a843",
  "Muddy"               = "#8B6914",
  "Emergent Macrophyte" = "#228B22"
)
site_info <- dplyr::mutate(site_info,
  color = dplyr::case_when(
    grepl("rocky",      DHT, ignore.case = TRUE) ~ "#808080",
    grepl("sandy",      DHT, ignore.case = TRUE) ~ "#d4a843",
    grepl("muddy",      DHT, ignore.case = TRUE) ~ "#8B6914",
    grepl("macrophyte", DHT, ignore.case = TRUE) ~ "#228B22",
    TRUE ~ "#555555"
  )
)
...
leaflet::leaflet() |>
  ...
  leaflet::addPolygons(
    data        = all_lakes,
    fillColor   = ~ifelse(catfish, "#ffe0b2", "#e3f2fd"),
    fillOpacity = 0.5,
    color       = "#2a2a2a",
    weight      = 1.5,
    label       = ~paste0(lake_name, ifelse(catfish, " (catfish present)", "")),
    popup       = ~paste0("<b>", lake_name, "</b>",
                          ifelse(catfish, "<br><i>Catfish present</i>", ""))
  ) |>
  leaflet::addCircleMarkers(
    ...
    fillOpacity = 0.9,
    weight      = 1.5,
    popup       = ~paste0("Site ", site_id, "<br>Lake: ", lake, "<br>Habitat: ", DHT)
  ) |>
  leaflet::addLegend(
    position = "bottomright",
    colors   = unname(dht_colors),
    labels   = names(dht_colors),
    title    = "Dominant habitat type",
    opacity  = 0.9
  )
...
if (isTRUE(getOption("thesis.is.html"))) {
  DT::datatable(biotic_vars, rownames = FALSE, filter = "top",
