...
This site accompanies the scientific work on: *Complex littoral habitats enhance freshwater crayfish populations in five volcanic lakes of Aotearoa New Zealand*.
...
#| fig-height: 4
#| dpi: 120
#| fig-cap: "Length–weight relationship for kōura (OCL length vs predicted weight)."
...
ggsave(
filename = file.path(fig_dir, "lengt_weight_plot.png"),
plot = lengt_weight_plot,
width = 170, height = 120, units = "mm", dpi = 1200
)
...
cor(
habitat_classification$Substrate_index,
...
select(Lake, Substrate_index, Slope_5m, Riparian_vegetation, Overhanging_trees, Wood_cover,
...
select(Lake, Presence_Eel, Presence_Common_smelt, Presence_Catfish, Presence_Goldfish, Presence_Kōaro, Presence_Trout) %>%
...
Env_summary_table %>% mutate(Var_type = "Env"),
Fish_summary_table %>% mutate(Var_type = "Fish")) %>%
select(Lake, Var_type, Variable, n, Mean, Median, Min, Max, CI_low, CI_high) %>%
arrange(Var_type, Variable, Lake)
...
#| fig-height: 7
#| dpi: 120
#| fig-cap: "Kōura presence, CPUE, and BPUE by lake."
...
```{r}
# Exploratory lake-level differences among lakes (GLMMs)
# Purpose: test whether kōura metrics differ among lakes (overall + pairwise contrasts)

#| label: lake_differences_glmm
#| code-fold: true
#| code-summary: "Show exploratory lake-level GLMM tests"
...
ggsave(
  filename = file.path(fig_dir, "Occupancy_panels.png"), plot = final_plot_occupancy,  width = 180, height = 120, units = "mm", dpi = 1200)
...
geom_path(colour = "black", linewidth = 0.5) +
geom_abline(slope = 1, intercept = 0, linetype = "dashed", colour = "black", linewidth = 0.4) +
annotate("text", x = 0.6, y = 0.1, label = paste0("AUC = ", round(auc_val, 3)), size = 3) +
labs(x = "False positive rate", y = "True positive rate") +
base_theme_bw

CPUE_panel_presence <- patchwork::wrap_plots(
list(as_plot(p_cpue_pres_smooths), as_plot(p_cpue_pres_param), as_plot(p_cpue_pres_roc)),
ncol = 3
)
CPUE_panel_presence
...
ggsave(
filename = file.path(fig_dir, "CPUE_panel_positives.png"),
plot = CPUE_panel_positives,
width = 130, height = 120, units = "mm", dpi = 1200
)
```
...
pred_pres_b <- predict(final_BPUE_pres, type = "response", exclude = exclude_if_RE(final_BPUE_pres))
...
geom_path(colour = "black", linewidth = 0.5) +
geom_abline(slope = 1, intercept = 0, linetype = "dashed", colour = "black", linewidth = 0.4) +
annotate("text", x = 0.6, y = 0.1, label = paste0("AUC = ", round(auc_val_b, 3)), size = 3) +
labs(x = "False positive rate", y = "True positive rate") +
base_theme_bw

BPUE_panel_presence <- patchwork::wrap_plots(
list(as_plot(p_BPUE_pres_smooths), as_plot(p_BPUE_pres_param), as_plot(p_BPUE_pres_roc)),
ncol = 3
)
BPUE_panel_presence
...
ggsave(
filename = file.path(fig_dir, "BPUE_panel_positives.png"),
plot = BPUE_panel_positives,
width = 100, height = 120, units = "mm", dpi = 1200
)
