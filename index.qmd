...
packages <- c("patchwork", "gratia","pROC","caret", "mgcv","tidyr","purrr", "corrplot","glmmTMB", "performance","tibble", "tidyverse", "dplyr","writexl", "readxl")
...
CPUE_BCUE_weighted_summary <- CPUE_BCUE_weighted %>%
  pivot_wider(names_from  = Species,
    values_from = c(Total_Individuals, Weighted_CPUE, Weighted_BCUE, Total_Weight,
...
fish_order_BCUE  <- c("Weighted_BCUE_Kōura","Weighted_BCUE_Eel","Weighted_BCUE_Kōaro","Weighted_BCUE_Common_smelt","Weighted_BCUE_Bullies","Weighted_BCUE_Catfish","Weighted_BCUE_Goldfish","Weighted_BCUE_Mosquitofish","Weighted_BCUE_Trout")
...
KBCUE_plot <- plot_koura_stats(M_C_data, "Weighted_BCUE_Kōura",     "Kōura BCUE",     test_type = "wilcox",  show_x_title = TRUE)
...
Koura_plots_0 <- KPRES_plot / KCPUE_plot / KBCUE_plot
Koura_plots_0
...
remove_high_vif_glmBI <- function(data, response, predictors, threshold = 5, protect_vars = character(0)) {
  ...
  repeat {
    ...
    model <- try(
      glm(fml, data = data, family = binomial(link = "logit"), method = "brglmFit"),
      silent = TRUE
    )
...
# VIF pruning (Gamma/Tweedie proxy) via glmmTMB (for non-binomial analyses)
remove_high_vif_glmmTMB <- function(data, response, predictors, threshold = 5, protect_vars = character(0)) {
  ...
  repeat {
    ...
    fml <- as.formula(paste(response, "~", paste(predictors, collapse = " + ")))
    model <- try(
      glmmTMB::glmmTMB(fml, data = data, family = glmmTMB::Gamma(link = "log")),
      silent = TRUE
    )
    if (inherits(model, "try-error")) break
    vif_data <- performance::check_collinearity(model)
...
ggsave(
  file.path(fig_dir, "Occupancy_panels.png"),
  final_plot_occupancy,
  width  = 14,
  height = 8,
  dpi    = 300
...
pred_fixed_bcue_pres <- setdiff(
  names(BData),
  c("BCUE_pos", id, "Weighted_BCUE_Kōura", "Weighted_BCUE_Koura", "Weighted_BCUE")
)

vif_bcue_pres <- remove_high_vif_glmBI(
  BData,
  "BCUE_pos",
  pred_fixed_bcue_pres,
  threshold    = vif_thresh,
  protect_vars = character(0)
)
kept_fixed_bcue_pres <- vif_bcue_pres$predictors

vars_step_bcue_pres <- c(kept_fixed_bcue_pres, if (INCLUDE_RE) id)
...
full_ml_bcue_pres <- mgcv::gam(
  as.formula(paste("BCUE_pos ~", rhs_bcue_pres)),
  data   = BData,
  family = binomial(link = "logit"),
  method = "ML",
  select = TRUE
...
repeat {
  ...
  m_now <- mgcv::gam(
    as.formula(paste("BCUE_pos ~", rhs_now)),
    data   = BData,
    family = binomial(link = "logit"),
    method = "ML",
    select = TRUE
...
# Clean test of LID random effect for BCUE presence
if (INCLUDE_RE) {
  m_withLID_b <- red_ml_bcue_pres
  tl_b <- attr(terms(red_ml_bcue_pres), "term.labels")
  tl_b_noLID <- tl_b[!grepl("LID", tl_b)]
  rhs_b_noLID <- if (length(tl_b_noLID)) paste(tl_b_noLID, collapse = " + ") else "1"
  f_b_noLID <- as.formula(paste("BCUE_pos ~", rhs_b_noLID))
...
# Final REML
final_bcue_pres <- mgcv::gam(
  formula(red_ml_bcue_pres),
  data   = BData,
  family = binomial(link = "logit"),
  method = "REML",
  select = TRUE
...
p_bcue_pres_param <- parametric_panel(
  final_bcue_pres,
  BData,
  alpha       = alpha_sig,
  exclude     = c("LID"),
...
pred_pres_b <- predict(
  final_bcue_pres,
  type    = "response",
  exclude = exclude_if_RE(final_bcue_pres)
)
...
BCUE_panel_presence <- patchwork::wrap_plots(
  list(
    as_plot(p_bcue_pres_smooths),
    as_plot(p_bcue_pres_param),
    as_plot(p_bcue_pres_roc)
  ),
  ncol = 3
)
BCUE_panel_presence
ggsave(
  file.path(fig_dir, "BCUE_panel_presence.png"),
  BCUE_panel_presence,
  width  = 10,
  height = 5,
  dpi    = 300
...
pred_fixed_bcue_pos <- setdiff(names(Bpos), c("Weighted_BCUE_Kōura", id))

vif_bcue_pos <- remove_high_vif_glmmTMB(
  Bpos,
  "Weighted_BCUE_Kōura",
  pred_fixed_bcue_pos,
  threshold    = vif_thresh,
...
vars_step_bcue_pos <- c(kept_fixed_bcue_pos, if (INCLUDE_RE) id)
...
full_ml_bcue_pos <- mgcv::gam(
  as.formula(paste("Weighted_BCUE_Kōura ~", rhs_bcue_pos)),
  data   = Bpos,
  family = Gamma(link = "log"),
  method = "ML",
  select = TRUE
...
repeat {
  ...
  m_now <- mgcv::gam(
    as.formula(paste("Weighted_BCUE_Kōura ~", rhs_now)),
    data   = Bpos,
    family = Gamma(link = "log"),
    method = "ML",
    select = TRUE
  ...
  # protect LID in positive BCUE model
...
# Clean test of LID random effect for positive BCUE
LID_test_bcue <- if (INCLUDE_RE) {
  m_withLID_bpos <- red_ml_bcue_pos
  tl_bpos <- attr(terms(red_ml_bcue_pos), "term.labels")
  tl_bpos_noLID <- tl_bpos[!grepl("LID", tl_bpos)]
  rhs_bpos_noLID <- if (length(tl_bpos_noLID)) paste(tl_bpos_noLID, collapse = " + ") else "1"
  f_bpos_noLID <- as.formula(paste("Weighted_BCUE_Kōura ~", rhs_bpos_noLID))
...
# Final REML
final_bcue_pos <- mgcv::gam(
  formula(red_ml_bcue_pos),
  data   = Bpos,
  family = Gamma(link = "log"),
  method = "REML",
  select = TRUE
...
p_bcue_pos_param <- parametric_panel(
  final_bcue_pos,
  Bpos,
  alpha       = alpha_sig,
  exclude     = c("LID"),
...
pred_pos_b <- predict(
  final_bcue_pos,
  type    = "response",
  exclude = exclude_if_RE(final_bcue_pos)
)
...
BCUE_panel_positives <- patchwork::wrap_plots(
  list(
    as_plot(p_bcue_pos_smooths),
    #as_plot(p_bcue_pos_param),
    as_plot(p_bcue_pos_scatter)
  ),
  ncol = 2
) 
...
ggsave(
  file.path(fig_dir, "BCUE_panel_positives.png"),
  BCUE_panel_positives,
  width  = 10,
  height = 5,
  dpi    = 300
