...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[color=white!80, anchor=south east,
    ...
    {Rocky shoreline with native riparian vegetation, Lake Rotoiti.};
\end{tikzpicture}
\null
\clearpage
```
...
.i <- file.path(dirname(knitr::current_input(dir = TRUE)), "03_koura_shoreline_habitats", "images", "ch3_cover.jpg")
if (file.exists(.i)) cat(sprintf('<div class="ch-hero-wrap"><img class="ch-hero-img" src="%s" alt=""><div class="ch-hero-fade"></div><div class="ch-hero-text"><span class="ch-eyebrow">Chapter 3</span><span class="ch-hero-title">Where do Kōura Live?</span></div></div>', knitr::image_uri(.i)))
```

# Chapter 3 Where do Kōura Live? {.unnumbered .ch-page-heading}
:::
...
fig_labels <- c("fig-koura-by-lake", "fig-occupancy-model", "fig-cpue-hurdle",
                 "fig-bpue-hurdle", "fig-length-weight-ch3", "fig-raw-data-all-predictors")
...
## Methods
### Study location
Study sites were selected in five lakes located in the Rotorua Te Arawa Lakes area in the North Island of Aotearoa. The Lakes Rotorua, Rotoiti, Rotoehu, Rotomā, and Ōkāreka are located on a warm-temperate volcanic plateau (280-355 m) where winters are mild and year-round temperatures typically vary from 4°C to 24°C. The lakes span a wide range of morphometric and trophic conditions from shallow polymictic eutrophic systems (Rotoehu, maximum depth 14 m) to deep stratified oligotrophic lakes (Rotomā, maximum depth 83 m). Shorelines are characterised by a mix of native riparian vegetation, exotic pasture, and low-density residential development, with several lakes also supporting stands of emergent macrophytes. Detailed lake characteristics and shoreline composition are summarised in @tbl-lake-overview. All five lakes have populations of kōura, and brown bullhead catfish was present in Lakes Rotorua and Rotoiti at time of sampling [@Kusabs2026].
...
![Map of the sampling locations with different dominant littoral habitat types classified. a) Overview map of the 5 lakes. Lakes where invasive catfish were detected are indicated. b) Zoom in of Lake Ōkāreka. c) Guidance map to show the lakes location (white square) on the North Island of Aotearoa New Zealand.](images/Natural_habitat_map.png){#fig-site-map width=80%}
:::
...
#| echo: false
#| eval: !expr knitr::is_html_output()
#| fig-cap: "Map of the 5 study lakes with sampling locations coloured by dominant littoral habitat type. Lakes with invasive catfish (Rotorua, Rotoiti) are shown with a warm fill; catfish-free lakes with a cool fill. Click a marker to see the site details."
ch3_dir <- if (dir.exists("03_koura_shoreline_habitats")) "03_koura_shoreline_habitats" else "."
.rds <- file.path(ch3_dir, "data/derived/widget_site_map.rds")
...
if (isTRUE(getOption("thesis.is.html")) && knitr::is_html_output()) {
  ch3_dir <- if (dir.exists("03_koura_shoreline_habitats")) "03_koura_shoreline_habitats" else "."
  .rds <- file.path(ch3_dir, "data/derived/widget_lake_overview.rds")
  ...
  tbl <- knitr::kable(lake_data,
    ...
    kableExtra::column_spec(3:8,   width = "0.9cm") %>%
    kableExtra::column_spec(9:10,  width = "1.5cm") %>%
    kableExtra::column_spec(11:14, width = "0.9cm")
...
if (isTRUE(getOption("thesis.is.html")) && knitr::is_html_output()) {
  ch3_dir <- if (dir.exists("03_koura_shoreline_habitats")) "03_koura_shoreline_habitats" else "."
  .rds <- file.path(ch3_dir, "data/derived/widget_biotic_table.rds")
  if (file.exists(.rds)) readRDS(.rds) else htmltools::tags$p(htmltools::tags$em("Table not yet built — render analysis.qmd first."), style = "color:#888;padding:1em")
} else if (knitr::is_latex_output()) {
...
![Kōura CPUE GAMM results and model performance. (A) Estimated smooth terms for surface water temperature, pH, and emergent native macrophytes, shown with partial effects and 95% confidence intervals. See [Fig. S3.2]{.content-visible when-format="pdf"}[Fig. S2]{.content-visible when-format="html"} for raw data underlying modelled relationships. (B) Predicted versus observed CPUE from the positive (Gamma) component of the hurdle model, with predictions excluding the random lake effect. The dashed 1:1 line indicates perfect agreement; annotated values report R² and RMSE to summarise model fit.](outputs/fig-cpue-hurdle.png){#fig-cpue-hurdle width=80% fig-pos="htbp"}
...
![Kōura BPUE GAMM results and model performance. (A) Estimated smooth terms for pH and emergent native macrophytes, shown with partial effects and 95% confidence intervals. See [Fig. S3.2]{.content-visible when-format="pdf"}[Fig. S2]{.content-visible when-format="html"} for raw data underlying modelled relationships. (B) Predicted versus observed BPUE from the positive (Gamma) component of the hurdle model, with predictions excluding the random lake effect. The dashed 1:1 line indicates perfect agreement; annotated values report R² and RMSE to summarise model fit.](outputs/fig-bpue-hurdle.png){#fig-bpue-hurdle width=65% fig-pos="htbp"}
