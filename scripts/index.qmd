---
title: "Littoral habitat structure drives freshwater crayfish populations"
...
This site accompanies the scientific work on: *Littoral habitat structure drives freshwater crayfish populations in five volcanic lakes of Aotearoa New Zealand*.
...
- **Title:** Littoral habitat structure drives freshwater crayfish populations in five volcanic lakes of Aotearoa New Zealand
...
ggsave(filename = file.path(fig_dir, "length_weight_plot.png"),plot = length_weight_plot,width = 170, height = 120, units = "mm", dpi = 1200)
...
write.csv(
  ...
  file = file.path(fig_dir, "EnvBio_summary_table.csv"),
  row.names = FALSE
)
...
ggsave(file.path(fig_dir, "Koura_plots.png"), Koura_plots, 
...
ggsave(file.path(fig_dir, "Raw_data_plots.png"), raw_plots,
...
all_models_table |>
  mutate(
    term = gsub("s\\(|\\)", "", term),  # clean up smooth term names
    p_value = ifelse(!is.na(p_value), format.pval(p_value, digits = 3, eps = 0.001), NA)
  ) |>
  select(-model) |>  # model shown as group header instead
  kable(
  ...
  pack_rows("Occupancy", 1, nrow(occ_table)) |>
  pack_rows("CPUE (positive sites)", nrow(occ_table) + 1, nrow(occ_table) + nrow(cpue_table)) |>
  pack_rows("BPUE (positive sites)", nrow(occ_table) + nrow(cpue_table) + 1, nrow(all_models_table))
